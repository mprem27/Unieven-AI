...
function Reels() {
  ...
  const [originalReels, setOriginalReels] = useState([]);
  ...
  useEffect(() => {
    if (reels.length > 0 && originalReels.length > 0) {
      if (currentIndex >= reels.length - 2) {
        let moreReels = shuffleArray(originalReels);
        
        // Prevent immediate duplicate when looping
        if (moreReels[0]._id === reels[reels.length - 1]._id && moreReels.length > 1) {
          const first = moreReels.shift();
          moreReels.push(first);
        }

        moreReels = moreReels.map(r => ({
          ...r,
          uniqueKey: `${r._id}-${Math.random().toString(36).substr(2, 9)}`
        }));
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.70 } // slightly lowered threshold for faster trigger during fast scrolls
    );
  ...
  return (
    <div className="h-[100dvh] bg-[#000] flex items-center justify-center overflow-hidden font-['SF_Pro_Display',system-ui,sans-serif] selection:bg-none">
      ...
      <div
        ref={containerRef}
        className="w-full max-w-[400px] h-[100dvh] overflow-y-scroll snap-y snap-mandatory scroll-smooth relative bg-black scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] overscroll-y-none will-change-transform"
      >
        {reels.map((reel, i) => {
          const textContent = reel.caption || reel.text || reel.description || "";
          const isExpanded = expandedCaption[i];
          ...
          return (
            ...
            >
              <video
                ...
                loop
                // 🔥 Critical fix for smooth scroll: only preload adjacent videos
                preload={Math.abs(currentIndex - i) <= 2 ? "auto" : "none"}
                muted={muted}
                playsInline
              ...
              <div className="absolute bottom-6 left-4 right-16 z-30 text-white flex flex-col gap-2.5 pb-2">
                ...
                {/* DYNAMIC TEXT DISPLAY - Expandable & Scrollable */}
                {textContent && (
                  <div className="pr-4 pointer-events-auto">
                    <p className={`text-white text-[14px] leading-snug drop-shadow-md font-normal whitespace-pre-wrap transition-all duration-300 ${isExpanded ? 'max-h-[30dvh] overflow-y-auto scrollbar-hide' : 'line-clamp-2'}`}
                    ...
                    {textContent.length > 70 && (
                      <button
                        ...
                        className="text-white/70 font-semibold text-[13px] hover:text-white mt-1 drop-shadow-md"
                      >
                ...
                <div className="flex items-center gap-2 text-white drop-shadow-md mt-1 pointer-events-none">
                  <FaMusic className="text-[12px]" />
                  <div className="overflow-hidden whitespace-nowrap max-w-[200px]">
                    <span className="inline-block text-[13px] font-medium animate-marquee">
