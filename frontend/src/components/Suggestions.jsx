...
function Suggestions({ users, currentUser, onRefreshSuggestions }) {
      ...
      await Promise.all(
        users.map(async (user) => {
          try {
            ...
            const isLocallyRequested = getLocalPending().includes(user._id);

            if (profile?.user) {
              const isFollowing = profile.user.isFollowing === true;
              const isRequested = profile.user.isRequested === true || isLocallyRequested;
            ...
            const isLocallyRequested = getLocalPending().includes(user._id);
            statusMap[user._id] = isLocallyRequested ? "requested" : null;
          }
        })
      );
  ...
  const handleConnect = async (userToConnect) => {
    ...
    try {
      ...
      // Remove instantly from feed view if provided
