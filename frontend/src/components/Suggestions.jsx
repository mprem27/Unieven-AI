...
function Suggestions({ users }) {
  ...
  // Track specific connection states: "accepted", "requested", or null/undefined
  ...
  const handleConnect = async (userId) => {
    try {
      ...
      // Map the exact response from your smart toggle backend
      if (res.message === "Unfollowed / Request cancelled") {
        setConnectionStatus((prev) => ({ ...prev, [userId]: null }));
      } else {
        // Sets it to either "accepted" (Connected) or "requested" (Requested)
        setConnectionStatus((prev) => ({ ...prev, [userId]: res.status }));
      }
    } catch (err) {
            ...
            >
              ...
              <div 
                ...
                className={`w-[68px] h-[68px] md:w-[72px] md:h-[72px] rounded-full p-[2.5px] shadow-sm mb-3 cursor-pointer hover:scale-105 transition-transform duration-300 ${
                  hasInteraction ? "bg-gray-200" : "bg-gradient-to-tr from-blue-400 to-purple-500"
                }`}
              >
              ...
              <button
                ...
                className={`w-full py-1.5 md:py-2 rounded-xl text-[12px] md:text-[13px] font-black transition-all duration-300 active:scale-95 flex justify-center items-center h-[34px] md:h-[36px] ${
                  hasInteraction
                    ? "bg-gray-100 text-gray-600 border border-gray-200" // Requested or Connected state
                    : "bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-500/20" // Default Connect state
                }`}
              >
                {loadingId === u._id ? (
                  <div className={`w-4 h-4 border-2 border-t-transparent rounded-full animate-spin ${hasInteraction ? 'border-gray-500' : 'border-white/50'}`}></div>
                ) : isConnected ? (
                  "Connected"
                ) : isPending ? (
                  "Requested"
